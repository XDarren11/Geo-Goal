          ...
          {leagues.map((league, i) => {
            ...
            return (
              <li key={league.id} className={`opacity-0 animate-in-up ${stagger} relative group`}>
                <Link to={`/leagues/${league.id}/results`} className="card-pitch block p-5 h-full">
                  <div className="flex items-center justify-between">
                    <h2 className="font-geo text-xl tracking-wide text-geo-green pr-4">
                      {league.name}
                    </h2>
                    <CalendarDaysIcon className="h-6 w-6 text-zinc-600 transition-colors group-hover:text-geo-green" />
                  </div>
