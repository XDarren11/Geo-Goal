...
export function LeagueInvitationCodeModal({
  leagueId,
  isOpen,
  onClose,
  ...
  const { mutate: generateCode, isPending } = useMutation({
    mutationFn: () => leagueInvitationAPI.generateCode(leagueId),
    onSuccess: (data) => {
      setCode(data.code);
  ...
  const { mutate: revokeCode, isPending: isRevoking } = useMutation({
    mutationFn: () => leagueInvitationAPI.revokeCode(leagueId),
    onSuccess: () => {
      setCode('');
      setCopied(false);
            ...
            <>
              <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg text-center">
                <p className="text-sm text-[var(--geo-text-muted)] mb-2">
                  Comparte este código con los entrenadores:
                </p>
