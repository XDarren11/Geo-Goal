...
export async function removeFavoriteByEntity(opts: {
  ...
  const { data } = await api.delete<{ removed: number }>(`${ACCOUNT}/favorites`, {
    params: opts,
  });
  return data;
}
...
export async function updateFavorite(
  id: number,
  opts: { label?: string | null; sortOrder?: number }
): Promise<FavoriteItem> {
  const { data } = await api.patch<FavoriteItem>(`${ACCOUNT}/favorites/${id}`, opts);
  return data;
}
